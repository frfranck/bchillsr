...
@app.route('/api/event/<prod>/<supp>/<rcv>/<cmdid>/<status>/<delay>',method='PUT')
@Enable_Cors
def serversideevent(prod='none',supp='none',rcv='none',cmdid='none',status='none',delay='0'):
    for u in wsusers:
        msg = prod + ';' + supp + ';' + rcv + ';' + cmdid + ';' + status + ';' + delay
        u.send(msg)
